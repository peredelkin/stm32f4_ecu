...
void crank_extrapolation_next_1_period(ecu_t* ecu,uint8_t vr_count) {
...
void crank_extrapolation_next_1_capture(ecu_t* ecu,uint8_t vr_count) {
    uint8_t next_1 = ecu_crank_vr_numb_normalization(vr_count + 1);
    //расчет следующего захвата
    ecu->crank.capture[next_1] = ecu->crank.capture[vr_count] + ecu->crank.period[next_1];
}

void crank_extrapolation_next_2_period(ecu_t* ecu,uint8_t vr_count) {
    uint8_t next_1 = ecu_crank_vr_numb_normalization(vr_count + 1);
...
void crank_extrapolation_next_2_capture(ecu_t* ecu,uint8_t vr_count) {
    uint8_t next_1 = ecu_crank_vr_numb_normalization(vr_count + 1);
    uint8_t next_2 = ecu_crank_vr_numb_normalization(vr_count + 2);
...
void crank_extrapolation_calc(ecu_t *ecu) {
    if (ecu->gap_found) {
        ...
        crank_extrapolation_next_1_period(ecu,vr_count);
        crank_extrapolation_next_1_capture(ecu,vr_count);
        crank_extrapolation_next_2_period(ecu,vr_count);
    }
}
