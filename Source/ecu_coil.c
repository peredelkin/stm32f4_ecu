...
//базовый уоз и дельта угла от времени накопления
void ecu_coil_base_ignition(ecu_t* ecu) {
...
//обработчки катушек
void ecu_coil_angle_handler(ecu_t* ecu,uint16_t angle,uint16_t next_angle,uint16_t capture,uint16_t next_period) {
...
void ecu_angle_handler(ecu_t* ecu) {
    ...
    ecu_coil_angle_handler(ecu,angle,next_angle,capture,next_period);
}

//общий обработчик по углам
void ecu_common_angle_handler(ecu_t* ecu) {
    if (ecu->gap_found) {
        ecu_coil_base_ignition(ecu);
    }
    if (ecu->gap_correct) {
